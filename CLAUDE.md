...
* 假设 Quant 是一名经验丰富的资深全栈工程师，熟悉：
  ...
  * 后端：Python、FastAPI、SQLAlchemy、pgSQL
...
| 类型       | 规范                 | 示例                                |
| ---------- | -------------------- | ----------------------------------- |
| 组件文件   | PascalCase           | `ShareSettingsModal.tsx`          |
| Hook 文件  | camelCase +`use`   | `useFileUpload.ts`                |
| Store 文件 | camelCase +`Store` | `documentCompareStore.ts`         |
| 工具文件   | camelCase            | `fileStorageService.ts`           |
| 类型文件   | kebab-case           | `document-compare-types.ts`       |
| 类/接口    | PascalCase           | `ShareItem`, `DocumentReadTask` |
| 变量/函数  | camelCase            | `handleSave`, `uploadedFiles`   |
...
| 类型     | 规范                     | 示例                                     |
| -------- | ------------------------ | ---------------------------------------- |
| 路由文件 | snake_case +`_routes`  | `document_read_routes.py`              |
| 模型文件 | snake_case               | `document_read_tasks.py`               |
| 服务文件 | snake_case +`_service` | `document_read_task_service.py`        |
| 表名     | snake_case（复数）       | `document_read_tasks`                  |
| 字段名   | snake_case               | `task_name`, `created_at`            |
| 类名     | PascalCase               | `DocumentReadTask`                     |
| 枚举     | PascalCase               | `TaskStatus`, `WorkflowVisibility`   |
| 函数名   | snake_case               | `create_task`, `get_user_permission` |
...
3. 偏好 **最小、可审阅的修改**：
   ...
   * 如需展示完整文件，应标明关键变更区域。
   * 坚持 Fail-Fast 原则，如果发生错误，立即显式抛出错误，尽量避免fallback。
4. 明确指出应该如何验证改动：
   * 建议运行哪些测试 / 命令；
...
6. 对于简单的代码修复或修改，直接展示修改后的代码即可，避免不必要的解释和文档。
...
* 所有业务模型继承 `BusinessModel`，自动获得 id、时间戳、用户追踪字段
* 权限服务继承 `BasePermissionService`，提供 `owner_permission()`、`public_permission()`、`no_permission()` 静态方法
* 前端使用统一设计系统 `styles/DesignSystem.ts`，使用语义化颜色而非硬编码
